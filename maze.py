...
class Maze():
    ...
    def _break_walls_r(self, i, j):
        ...
        while True:
            ...
            direction_i = random.randrange(len(neighbors))
            next_index = neighbors[direction_i]
    ...
    def _solve_r(self, i, j):
        ...
        if i > 0 and not self._cells[i - 1][j].visited and not current_cell.has_left_wall:
            current_cell.draw_move(self._cells[i - 1][j], undo=False)
            visiting = self._solve_r(i - 1, j)
            if visiting == True:
                return True 
            else:
        ...
        if j > 0 and not self._cells[i][j - 1].visited and not current_cell.has_top_wall:
            current_cell.draw_move(self._cells[i][j - 1], undo=False)
            visiting = self._solve_r(i, j - 1)
            if visiting == True:
                return True 
            else:
